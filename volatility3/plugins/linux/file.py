...
from volatility3.plugins.linux import docker_mount as mount_plugin
from volatility3.plugins.linux import docker_pslist
...
class ListFiles(interfaces.plugins.PluginInterface):
    ...
    def get_file_info(cls,
        ...
        if inode is not None:
            ...
            # get file times
            ctime = inode.get_change_time()
            if not isinstance(ctime, renderers.UnparsableValue):
                created = int(ctime.timestamp())
            mtime = inode.get_modification_time()
            if not isinstance(mtime, renderers.UnparsableValue):
                modified = int(mtime.timestamp())
            atime = inode.get_access_time()
            if not isinstance(atime, renderers.UnparsableValue):
                accessed = int(atime.timestamp())
    ...
    def get_dentries(cls,
        ...
        if pid_filter is None:
            pid_filter = docker_pslist.PsList.create_pid_filter([1])
    ...
    def _generator(self):
        ...
        if self.config.get('mount') and not pids:
            ...
            pids = []
            for task in docker_pslist.PsList.list_tasks(self.context, self.config['kernel']):
                pids.append(task.pid)
        
        # build PID filter
        if pids:
            pid_filter = docker_pslist.PsList.create_pid_filter(pids)
        else:
            pid_filter = None
