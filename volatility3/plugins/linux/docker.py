...
from volatility3.plugins.linux import docker_pslist, docker_mount, ifconfig
...
class Detector():
    ...
    def __init__(self, context, vmlinux, tasks_list) -> None:
        ...
        self.mounts = docker_mount.Mount.get_all_mounts(
    ...
    def generate_detection_list(self):
        ...
        for task, mnt in self.mounts:
            ...
            _id, _pid, _devname, path, _abs_type, fstype, _access, _flags = docker_mount.Mount.get_mount_info(
...
class Ps():
    ...
    def get_container_id(self, container_pid):
        ...
        pid_filter = docker_pslist.PsList.create_pid_filter([container_pid])
        process_mounts = docker_mount.Mount.get_mounts(
            self.context, self.vmlinux.name, pid_filter)  # Extract mounts for this process
        process_mounts = [docker_mount.Mount.get_mount_info(self.context, self.vmlinux.name, mnt, task=task)
    ...
    def get_init_task_cap(self):
        ...
        if init_task.pid == 1:
            init_task_info = docker_pslist.PsList.get_task_info(
        ...
        for task in self.tasks_list:
            for pid in containers_pids:
                if task.pid == pid:
                    ...
                    task_info = docker_pslist.PsList.get_task_info(
...
class InspectMounts():
    ...
    def generate_mounts_list(self, extended=True):
        ...
        It returns: container pid, container_id and details about the mount taken from linux.docker_mount.
        """

        # For each container, check for unusual mounts
        for pid in self.containers_pids:
            pid_filter = docker_pslist.PsList.create_pid_filter([pid])
            process_mounts = docker_mount.Mount.get_mounts(
                self.context, self.vmlinux.name, pid_filter)  # Extract mounts for this process
            # Extract mount info for each mount point
            process_mounts = [docker_mount.Mount.get_mount_info(
...
class Docker(interfaces.plugins.PluginInterface):
    ...
    def get_requirements(cls) -> List[interfaces.configuration.RequirementInterface]:
        return [requirements.ModuleRequirement(name='kernel',
                ...
                requirements.PluginRequirement(name='docker_pslist',
                                               plugin=docker_pslist.PsList,
                                               version=(2, 0, 0)),
                requirements.PluginRequirement(name='docker_mount',
                                               plugin=docker_mount.Mount,
                                               version=(1, 0, 0)),
    ...
    def _generator(self):
        ...
        # Generate tasks list from memory using linux.docker_pslist
