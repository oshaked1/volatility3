...
class BpfPslist(PluginInterface):
    ...
    def list_bpf_procs(
        ...
        for fd in fds_generator:
            # first iteration
            if saved_task is None or saved_pid is None:
                saved_pid = int(fd.task.pid)
                saved_task = fd.task

            # next task
            if int(fd.task.pid) != saved_pid:
            ...
            # add if BPF object
            filp: ObjectInterface = fd.fd_fields[1]
            vollog.debug(
                f"Checking: pid {int(fd.task.pid)}, comm {utility.array_to_string(fd.task.comm, 16)}, fd {fd.fd_fields[0]}, op {hex(int(filp.f_op))} {vmlinux.get_symbols_by_absolute_location(int(filp.f_op))}"
            )
